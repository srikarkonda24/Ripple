...
import {
  getConceptContext,
  validateSearchTerm,
} from "./tools/getConceptContext.js";
...
function toolError(message: string): {
  ...
  const payload: ToolErrorOutput = { error: message };
...
server.registerTool(
  ...
  {
    description:
      "Returns all product concepts with their name and description.",
    inputSchema: {},
  },
  async () => {
    try {
...
server.registerTool(
  ...
  {
    description:
      "Searches concepts by name or description and returns matching concepts with related source files.",
    inputSchema: {
      searchTerm: z
        .string()
